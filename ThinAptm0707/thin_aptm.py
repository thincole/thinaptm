...
class App(ctk.CTk):
    ...
    def _build_queue(self):
        ...
        ctk.CTkLabel(bar, text="📤 Upload/TK:", font=("", 11, "bold"), text_color=T1).pack(side="left", padx=(12, 2))
        self.spn_upload_threads = ctk.CTkEntry(bar, width=42, height=30, font=("", 12, "bold"), justify="center")
        self.spn_upload_threads.pack(side="left", padx=(0, 4))
        self.spn_upload_threads.insert(0, str(self.settings.get("upload_threads_per_acc", 3)))
    ...
    def _stop_single_account(self, email):
        """Dừng 1 tài khoản cụ thể — hủy tích chọn trong tab 'Tài khoản' và cho tài khoản đó dừng tạo video."""
        target_acc = None
        email_clean = (email or "").strip().lower()
        for a in self.accounts:
            a_em = (a.get("email") or a.get("id") or "").strip().lower()
            if a_em == email_clean or a_em.startswith(email_clean) or email_clean.startswith(a_em):
                target_acc = a
                break

        if target_acc:
            target_acc["enabled"] = False
            acc_name = target_acc.get("email") or target_acc.get("id") or email
            self._log(f"⏹ Đã dừng tài khoản [{acc_name}] (đã bỏ chọn 'Dùng' trong tab Tài khoản)")

        # Nếu tài khoản đang nằm trong pool đang chạy, cho tạm nghỉ vô thời hạn để ngắt luồng
        all_states = (getattr(self, "_sv_pool_states", None) or []) + (getattr(self, "_sp_pool_states", None) or []) + (getattr(self, "_pool_states", None) or [])
        for st in all_states:
            st_email = (st.email or "").strip().lower()
            if st_email == email_clean or st_email.startswith(email_clean) or email_clean.startswith(st_email):
                st.rest(86400 * 365, "user_stopped")
                if getattr(self, "proxy_pool", None):
                    try: self.proxy_pool.release(st.email)
                    except: pass

        # Cập nhật lại UI bảng Tài khoản nếu đã khởi tạo
        if hasattr(self, "_refresh_acc"):
            try: self.after(0, self._refresh_acc)
            except: pass
        ...
        try:
            ...
            if sig != self._pool_row_sig:
                ...
                if not states:
                    ...
                    cols = [("Tài khoản", 140), ("✅ Xong", 50), ("❌ Lỗi", 50),
                            ("⚡ Tạo", 45), ("🚀 Tốc độ", 60), ("Trạng thái", 100), ("Hành động", 70)]
                    ...
                    for i, s in enumerate(states):
                        ...
                        sl = ctk.CTkLabel(row, text="", font=("", 11), text_color=GR, width=100, anchor="w"); sl.pack(side="left", padx=(2, 0))
                        ctk.CTkButton(row, text="⏹ Dừng", width=55, height=24, fg_color="#E57373", hover_color="#EF5350", font=("", 10, "bold"), command=lambda em=s.email: self._stop_single_account(em)).pack(side="left", padx=(4, 0))
            ...
            for s in states:
                ...
                if rem > 0:
                    ...
                    elif s.rest_reason == "user_stopped":
                        r["s"].configure(text="⏹ Đã dừng", text_color=RD)
                    else:
    ...
    def _start(self):
        ...
        try:
            up_threads = max(1, int(self.spn_upload_threads.get().strip()))
        except Exception:
            up_threads = 3
        wpa = up_threads * 2
        self._user_submit_max = float(up_threads * 2)
    ...
    def _run(self, accs_placeholder, todo, wpa):
        ...
        try:
            ...
            try:
                user_up_threads = max(1, int(self.spn_upload_threads.get().strip()))
            except Exception:
                user_up_threads = 3
            n_upload_threads = max(1, len(accs) * user_up_threads)
            upload_sem = threading.Semaphore(n_upload_threads)   # số luồng upload = số TK × số luồng/TK do người dùng gán
            jobq = queue.Queue()
            for j in todo:
                j["_cycles"] = 0
                jobq.put(j)
    ...
    def _build_shopee(self):
        ...
        ctk.CTkLabel(row1, text="📤 Upload/TK:", font=("", 11, "bold"), text_color=T1).pack(side="left", padx=(12, 2))
        self._sp_upload_threads = ctk.CTkEntry(row1, width=42, height=28, font=("", 12, "bold"), justify="center")
        self._sp_upload_threads.pack(side="left", padx=(0, 4))
        self._sp_upload_threads.insert(0, str(self.settings.get("shopee_upload_threads_per_acc", 3)))
    ...
    def _sp_update_pool(self):
        ...
        try:
            ...
            if sig != self._sp_pool_row_sig:
                ...
                if states:
                    cols = [("Tài khoản", 140), ("✅ Xong", 50), ("❌ Lỗi", 50),
                            ("⚡ Tạo", 45), ("🚀 Tốc độ", 60), ("Trạng thái", 100), ("🌐 Proxy", 150), ("Hành động", 70)]
    ...
    def _shopee_start(self):
        ...
        try:
            up_threads = max(1, int(self._sp_upload_threads.get().strip()))
        except Exception:
            up_threads = 3
        _sp_cached_wpa = up_threads * 2
        _sp_cached_submit_max = float(up_threads * 2)
        ...
        def work():
            ...
            # ── Shared Job Queue ──
            try:
                user_up_threads = max(1, int(self._sp_upload_threads.get().strip()))
            except Exception:
                user_up_threads = 3
            n_upload_threads = max(1, len(accs) * user_up_threads)
            upload_sem = threading.Semaphore(n_upload_threads)   # số luồng upload = số TK × số luồng/TK do người dùng gán
    ...
    def _build_server_video(self):
        ...
        ctk.CTkButton(btn_row, text="📂 Mở thư mục",
                                           command=lambda: os.startfile(self._sv_outdir.get().strip()) if os.path.isdir(self._sv_outdir.get().strip()) else None,
                                           fg_color="#5f6368", height=42, width=110).pack(side="left")

        ctk.CTkLabel(btn_row, text="📤 Upload/TK:", font=("", 11, "bold"), text_color=T1).pack(side="left", padx=(12, 2))
        self._sv_spn_upload_threads = ctk.CTkEntry(btn_row, width=42, height=32, font=("", 12, "bold"), justify="center")
        self._sv_spn_upload_threads.pack(side="left", padx=(0, 4))
        self._sv_spn_upload_threads.insert(0, str(self.settings.get("sv_upload_threads_per_acc", 3)))
    ...
    def _sv_start(self):
        ...
        try:
            up_threads = max(1, int(self._sv_spn_upload_threads.get().strip()))
        except Exception:
            up_threads = 3
        _sv_cached_wpa = up_threads * 2
        _sv_cached_submit_max = float(up_threads * 2)
        ...
        self.settings["sv_upload_threads_per_acc"] = int(self._sv_spn_upload_threads.get().strip() or "3")
        ...
        def work():
            ...
            error_count = [0]
            try:
                user_up_threads = max(1, int(self._sv_spn_upload_threads.get().strip()))
            except Exception:
                user_up_threads = 3
            n_upload_threads = max(1, len(states) * user_up_threads)
            upload_sem = threading.Semaphore(n_upload_threads)  # Tổng luồng upload = Số TK đang chạy x số luồng/TK
            self._sv_log_msg(f"📤 Khởi tạo {n_upload_threads} luồng upload song song (Số TK đang chạy: {len(states)} × {user_up_threads})")
    ...
    def _on_closing(self):
        ...
        try:
            ...
            s = {
                ...
                # Tạo + Tốc độ settings (tự động = 2 * Upload/TK)
                "workers_per_account": int(self.spn_upload_threads.get().strip() or "3") * 2,
                "submit_max": int(self.spn_upload_threads.get().strip() or "3") * 2,
                "upload_threads_per_acc": int(self.spn_upload_threads.get().strip() or "3"),
                "shopee_upload_threads_per_acc": int(self._sp_upload_threads.get().strip() or "3"),
                # Telegram settings
            ...
            if hasattr(self, '_sv_url'):
                s.update({
                    ...
                    "sv_workers_per_account": int(self._sv_spn_upload_threads.get().strip() or "3") * 2,
                    "sv_submit_max": int(self._sv_spn_upload_threads.get().strip() or "3") * 2,
