...
def generate(model: str, prompt: str, images: list | None = None, **params) -> list[str]:
...
| `size` | `string` | no | `"auto"`, `"1:1"`, `"3:4"`, `"9:16"`, `"4:3"`, `"16:9"`, `"2:3"`, `"3:2"`, `"21:9"`; Tỉ lệ |
...
| `count` | `integer` | no | range `1–8`; default `1`; Số ảnh |
| `aspect_ratio` | `select` | no | `"16:9"`, `"9:16"`, `"1:1"`, `"4:3"`, `"3:4"`; default `"16:9"`; Tỉ lệ |
...
| `aspect_ratio` | `select` | no | `"16:9"`, `"9:16"`; default `"9:16"`; Tỉ lệ |
| `count` | `number` | no | range `1–4`; default `1`; Số video |
| `duration` | `select` | no | `"4s"`, `"6s"`, `"8s"`; default `"8s"`; Thời lượng |
...
LƯU Ý MODEL SEEDANCE:Nguồn: Seedance 2.5/2.0 Fast từ Dola.Tham chiếu người thật: không hỗ trợ (thỉnh thoảng vẫn tạo được).Kiểm duyệt: Dola duyệt gắt, cần tối ưu prompt (có hướng dẫn). Dola hay gặp lỗi vặt hoặc bị duyệt nhầm, tạo lại vài lần sẽ qua.Khuyên dùng: Phù hợp nhất để tích hợp dự án riêng, nên có sẵn cơ chế retry để chạy ổn định.


Model ids in this family: `seedance_2.0_fast`, `seedance_2.5`.
...
| 404 | `model_not_found` | Unknown model id; the message lists the valid ids |
