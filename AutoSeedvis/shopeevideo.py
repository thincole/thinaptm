...
_CONT_EN = (
    ...
    "- ITEM PERSISTENCE & HAND LOCK: Any object the character holds or wears at the start must remain naturally present throughout. The character has strictly TWO arms attached to shoulders and exactly TWO normal human hands with five fingers each. ABSOLUTELY NO third arm, NO disembodied hands reaching from off-camera to hold the product, NO floating limbs, NO extra fingers, NO fused or mutated digits, NO limb deformations. The presenter's own hands must do all the holding.\n"
    "\n"
    "[NEGATIVE PROMPTS]\n"
    ...
    "- No anatomical anomalies, no extra limbs, no extra hands, no third arm, no disembodied hands, no floating hands, no extra fingers, no weird hand deformations, no fused digits, no missing fingers, no phantom limbs.\n"
...
_CONT_VI = (
    ...
    "- KHÓA GIẢI PHẪU & BÀN TAY (HAND & ANATOMY LOCK): Nhân vật có đúng HAI cánh tay nối liền với vai và HAI bàn tay người bình thường với đúng 5 ngón mỗi bàn tay. TUYỆT ĐỐI KHÔNG sinh thêm tay, KHÔNG có cánh tay thứ 3, KHÔNG có bàn tay ma từ ngoài khung hình thò vào giữ sản phẩm, không thừa ngón tay, không biến dạng chi. Mọi thao tác cầm nắm phải do chính bàn tay nhân vật thực hiện.\n"
    "\n"
    ...
    "- TUYỆT ĐỐI KHÔNG sinh thêm tay, KHÔNG sinh thêm chi, KHÔNG có cánh tay thứ 3 (third arm), KHÔNG bàn tay lơ lửng vô chủ (disembodied hand), KHÔNG thừa ngón tay, KHÔNG ngón tay dị tật/dính liền (mutated/fused fingers), KHÔNG biến dạng bàn tay, KHÔNG chi thừa.\n"
...
def _apply_pov_or_unbox_transform(prompt, review_style, lang="en", force_pov=False):
    """Nếu review_style là POV/Unboxing hoặc force_pov=True, loại bỏ hoàn toàn mô tả người mẫu nữ / khuôn mặt,
    thay bằng chỉ thị POV chỉ quay bàn tay và sản phẩm trên mặt bàn."""
    style_str = str(review_style or "").lower()
    if not force_pov and not any(k in style_str for k in ("pov", "unbox", "đập hộp", "góc nhìn thứ nhất", "demo", "công dụng")):
        return prompt
...
def build_video_prompts(product_name, scene_en, duration_sec=16, lang="en", review_style="Random", content_style="Review kho hàng", no_model=False, image_path=None):
    ...
    if no_model:
        if review_style in ("🎲 Random", "Random") or not review_style or "random" in str(review_style).lower() or review_style in ("Review kho hàng", "Ngồi Review", "Review tự nhiên", "UGC Authentic", "UGC", "So Sánh/Đánh Giá"):
            review_style = random.choice(["POV", "Unboxing", "Demo công dụng"])
    elif review_style in ("🎲 Random", "Random") or not review_style or "random" in str(review_style).lower():
    ...
    for i, idx in enumerate(indices):
        ...
        prompt = _apply_pov_or_unbox_transform(prompt, target_style or review_style, lang=lang, force_pov=no_model)
